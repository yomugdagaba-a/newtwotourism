...
-- AddForeignKey (only if TourismPlace table exists)
DO $$
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'TourismPlace') THEN
        ALTER TABLE "TourismView" ADD CONSTRAINT "TourismView_tourismPlaceId_fkey" 
        FOREIGN KEY ("tourismPlaceId") REFERENCES "TourismPlace"("id") ON DELETE CASCADE ON UPDATE CASCADE;
    END IF;
END $$;
