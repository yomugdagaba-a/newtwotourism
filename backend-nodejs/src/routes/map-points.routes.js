...
// GET /api/map-points/tourism/:tourismPlaceId
...
// GET /api/map-points/road/:roadInfoId
router.get('/road/:roadInfoId', async (req, res, next) => {
  try {
    const points = await prisma.mapPoint.findMany({ where: { roadInfoId: parseInt(req.params.roadInfoId) } });
    res.json(points);
  } catch (err) { next(err); }
});
