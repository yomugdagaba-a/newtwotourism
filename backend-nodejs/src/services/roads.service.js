...
async function create(data) {
  ...
  const road = await prisma.roadInfo.create({ data: { tourismPlaceId: parseInt(data.tourismPlaceId), ...fromRoadDto(data, null) } });
...
async function getByTourism(tourismPlaceId) {
  const roads = await prisma.roadInfo.findMany({ where: { tourismPlaceId: parseInt(tourismPlaceId) } });
...
async function update(id, data) {
  const road = await prisma.roadInfo.findUnique({ where: { id } });
  if (!road) throw Object.assign(new Error('Road not found'), { status: 404 });
  const updated = await prisma.roadInfo.update({ where: { id }, data: fromRoadDto(data, road) });
