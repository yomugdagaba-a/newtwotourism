...
class EmailGmailService {
  ...
  async sendEmail(to, subject, html) {
    ...
    if (!transporter) { console.error(`❌ Email not sent to ${to}: Gmail credentials not configured`); return false; }
    console.log(`📧 Sending email to=${to} subject="${subject}"`);
    try {
      const sender = this._getSenderInfo();
      const info = await transporter.sendMail({ from: `"${sender.name}" <${sender.email}>`, to, subject, html });
...
module.exports = new EmailGmailService();
