...
import { AdminHotelService, AdminImageUploadService } from "@/services/admin.service";
...
export default function HotelImagesPage() {
  ...
  const loadImages = async () => {
    ...
    try {
      ...
      const [data, mainUrl] = await Promise.all([
        AdminHotelService.getHotelImages(token, hotelId),
        AdminHotelService.getHotelMainImageUrl(token, hotelId),
      ]);
      setImages(data);
  ...
  const handleAdd = async () => {
    if (!token) return;
    const pendingFile = (window as any).__pendingHotelGalleryFile as File | undefined;
    ...
    try {
      ...
      if (pendingFile) {
        imageUrl = await AdminImageUploadService.uploadHotelGalleryImage(token, hotelId, pendingFile, newTitle || undefined, newDescription || undefined);
      } else {
        await AdminHotelService.addHotelImage(token, hotelId, { imageUrl, title: newTitle || undefined, description: newDescription || undefined });
      }
      (window as any).__pendingHotelGalleryFile = undefined;
  ...
  const handleDelete = async (imageId: number) => {
    ...
    try {
      ...
      await AdminHotelService.deleteHotelImage(token, hotelId, imageId);
  ...
  const handleSetMain = async (img: HotelImage) => {
    if (!token) return;
    try {
      setActionLoading(img.id);
      await AdminHotelService.setMainHotelImage(token, hotelId, img.imageUrl);
  ...
  const handleEdit = async () => {
    ...
    const pendingFile = (window as any).__pendingHotelEditFile as File | undefined;
    ...
    try {
      ...
      if (pendingFile) {
        ...
        imageUrl = await AdminImageUploadService.uploadHotelGalleryImage(token, hotelId, pendingFile, editTitle || undefined, editDescription || undefined);
        // Update the image record with new URL
        await AdminHotelService.updateHotelImage(token, hotelId, editingImage.id, {
          imageUrl,
      ...
      (window as any).__pendingHotelEditFile = undefined;
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-base font-black text-gray-900">Gallery Images — Hotel #{hotelId}</h1>
        </div>
        <button onClick={() => setShowAddForm(true)}
          className="bg-white text-purple-600 border-2 border-purple-600 px-4 py-2 rounded-lg text-sm font-bold hover:bg-purple-50 flex items-center gap-2">
          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div className="max-w-5xl mx-auto px-4 py-6">
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-12 text-center shadow-sm">
            ...
            <p className="text-gray-400 text-xs mt-1">Click "Add Image" to add gallery images for this hotel</p>
          </div>
        ) : (
                ...
                <div className="flex items-center gap-2">
                  <input id="hotel-gallery-input" type="file" accept="image/jpeg,image/png,image/gif,image/webp" className="hidden"
                    onChange={(e) => {
                      ...
                      if (!file) return;
                      (window as any).__pendingHotelGalleryFile = file;
          ...
          <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-md border border-gray-200">
            ...
            <div className="space-y-3">
              <div>
                ...
                {editUrl && (
                  <div className="mb-3 relative group">
                    ...
                    <button
                      ...
                      onClick={() => {
                        setEditUrl('');
                        (window as any).__pendingHotelEditFile = null;
                      }}
                      className="absolute top-2 right-2 bg-red-500 hover:bg-red-600 text-white rounded-full p-1.5 shadow-lg transition-all"
                      title="Remove image"
                    >
                ...
                {/* Upload/Change button */}
                <div className="flex items-center gap-2">
                  <input id="hotel-edit-input" type="file" accept="image/jpeg,image/png,image/gif,image/webp" className="hidden"
                    onChange={(e) => {
                      const file = e.target.files?.[0];
                      if (!file) return;
                      (window as any).__pendingHotelEditFile = file;
                  ...
                  <button
                    type="button"
                    onClick={() => document.getElementById('hotel-edit-input')?.click()}
                    className="text-blue-600 hover:text-blue-800 text-sm font-medium transition-colors flex items-center gap-1.5"
                  >
                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
            ...
            <div className="flex gap-2 mt-4 justify-end">
              <button onClick={() => { setEditingImage(null); (window as any).__pendingHotelEditFile = undefined; }}
                className="px-4 py-2 text-xs font-bold text-gray-600 hover:bg-gray-100 rounded-lg">Cancel</button>
