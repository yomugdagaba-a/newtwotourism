...
const TourismCard: React.FC<Props> = ({ tourism, onClick }) => {
  ...
  // Translate dynamic DB content
  const translatedName = useTranslateText(tourism.name);
  const translatedKebele = useTranslateText(tourism.kebele);
  const translatedWereda = useTranslateText(tourism.wereda || "North Wollo");
  const translatedCategory = useTranslateText(tourism.category);
      ...
      <div className="relative h-64 flex-shrink-0 overflow-hidden bg-gradient-to-br from-gray-50 to-gray-100 group-hover:brightness-105 transition-all duration-500">
        {tourism.imageUrl ? (
          <img
            ...
            onError={(e) => { (e.target as HTMLImageElement).src = "/images/tourism1.jpg"; }}
            loading="lazy"
          />
        ) : (
          <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-gray-100 to-gray-200">
            <div className="text-center p-8">
              ...
              <span className="text-gray-500 text-sm font-medium block">{t("tourism.noImage")}</span>
            </div>
          </div>
        )}
      ...
      <div className="p-6 flex-1 flex flex-col justify-between">
        ...
        <div className="flex items-center justify-between pt-4 border-t border-gray-100 mt-auto">
          <span className="text-sm text-gray-500 font-medium">
            {translatedKebele ? `${translatedKebele}, ` : ""}{translatedWereda}
          </span>
